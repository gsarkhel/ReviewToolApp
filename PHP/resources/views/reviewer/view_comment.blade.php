...
{{-- {{dd($filter_page_no)}} --}}
    ...
    <div class="container table-container">
      <form id="filter-form" method="GET" action="{{ route('reviewer.view_comments', ['id' => $course_id]) }}">
        @csrf
        <!-- Status Filter Dropdown -->
        <input id="show" name="show" type="hidden" value="{{Request::get('show')}}"/>
        <input id="status" name="status" type="hidden" value="all"/>
        <input id="module_no" name="module_no" type="hidden" value="{{$filter_module_no}}" />
        <input id="page_no" name="page_no" type="hidden" value="{{$filter_page_no}}" />
    ...
    <div class="mb-3">
        <button type="submit" class="btn btn-gradient w-100 mt-2">Filter</button>
          ...
          <div class="mb-3">
            <button id="course" type="button" class="custom-filter btn {{Request::get('show')==="course" ? 'btn-gradient':'normal-btn'}}  w-100 mt-2">Course</button>
        </div>
        <div class="mb-3">
          <button id="module" type="button" class="custom-filter btn {{Request::get('show')==="module" ? 'btn-gradient':'normal-btn'}} w-100 mt-2">Module</button>
      </div>
      <div class="mb-3">
        <button id="screen" type="button" class="custom-filter btn {{Request::get('show')==="screen" ? 'btn-gradient':'normal-btn'}} w-100 mt-2">Screen</button>
    </div>
          <!-- Filter by Status -->
      ...
      document.addEventListener("DOMContentLoaded", function () {
        ...
        updateButtons.forEach((btn) => {
          btn.addEventListener("click", function () {
            ...
            fetch(baseUrl+`/get-comment-history/${courseId}/${commentId}`)
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        let baseUrl = "{{ url('/') }}";
                        document.getElementById("editcommentform").action = `${baseUrl}/reviewer/editcomment/${comment.comment.id}/${courseId}`;
    ...
    <script>
      ...
      // Monitor for parent tab's presence
      // setInterval(() => {
      //     if (!parentAlive) {
      //         window.close();
      //     }
