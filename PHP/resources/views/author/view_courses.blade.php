...
<head>
    ...
    <title>Author View Course</title>
...
<body>
                    ...
                    <tr data-comment="Page" data-course="Sample Course A" data-reviewer="John Doe"
                        ...
                        <td>
                            <div class="tbl_control">
                                <a href="{{ route('author.detailcourse', ['id' => $course->uuid]) }}" class="detailcourse-btn" id="courseLink">
                                ...
                                <a href="#" onclick="copyToClipboard(event)">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" 
                                        viewBox="0 0 24 24" fill="none" stroke="#de4f12" stroke-width="2" 
                                        stroke-linecap="round" stroke-linejoin="round">
	...
	<script>
		function copyToClipboard(event) {
		    ...
		    const modifiedLink = linkElement.href.replace("/author/", "/reviewer/");

		    if (navigator.clipboard && navigator.clipboard.writeText) {
		        // Modern clipboard API
		        navigator.clipboard.writeText(modifiedLink).then(() => {
		            showPopup(modifiedLink, true);
		        }).catch(() => {
		            fallbackCopy(modifiedLink);
		        });
		    } else {
		        // Fallback method
		        fallbackCopy(modifiedLink);
		...
		function fallbackCopy(text) {
		    ...
		    showPopup(text, success); // Show popup based on success
		}

		function showPopup(link, copied) {
		    Swal.fire({
		        title: copied ? "Link Copied!" : "Copy Link",
		        html: `<p style="word-wrap: break-word;">${link}</p>`,
		        icon: copied ? "success" : "info",
		        showConfirmButton: false,
		        timer: 5000,
		        position: "top-end", // Adjusted for better visibility
		        toast: true,
