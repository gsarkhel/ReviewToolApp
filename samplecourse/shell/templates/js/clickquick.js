...
function fnLoadPageExternalData(eData){
	$.each(eData, function(key, value){
		if(key=="audioTransText"){
			...
			quickpdf = value.refpdf;
			quickcontent = value.refcontent;
		 ...
		 $(".quickRefText").hide();
		  $(".pdfContent").hide();	 
			...
			totalclicks++;
				...
				callerId = $(this).attr("id").split("_")[1];
...
var vWidth = $('.floatRight').width();
	...
	video_Player = new MediaElementPlayer('#video_player', {
		  ...
		  $(".pdfContent").show();	 
				 	...
				 	$("#page").find('.pdfContent a').css("pointer-events", "none");
					$("#page").find('.pdfContent a').css("opacity", "0.5");
				 	var pdfCont =  fnLoadExternalCont(quickpdf[callerId-1])
				console.log(quickcontent[callerId-1])
				$(".quickRefText").html(quickcontent[callerId-1]) 
				...
				mediaElement.addEventListener('ended', function(e) {
				  $("#page").find('.pdfContent a').css("pointer-events", "auto");
					$("#page").find('.pdfContent a').css("opacity", "1");
					$('.videoContainer .playPauseAudioBtn').removeClass('pause').addClass('play');
					video_Player.pause();
				}, false);
