...
function fnLoadPageExternalData(eData){
	...
	$(".inner_height_drophotspot").css("overflow-y","auto");
	}
  ...
  $(".inner_height_drophotspot").css("overflow-y","auto !important");*/
	fnLoadQuestion();
	...
	$(document).click(function () {
		$(".selectBoxDnD").removeClass("selectBoxDnDHidden");
	...
	$(".Rst_btn").click(function() {
		if($(this).hasClass("cyudisabled")){
			...
			fnselectreset();
			submitEnable=false;
			$(".elips").find("span.optHTxt").html(selectOptTxt);
			$("#submit").addClass("cyudisabled");
			$(".Rst_btn").addClass("cyudisabled");
		}
	})
    $(".cyuPopClsBtn").click(function(e) {
        ...
        if (attempt == 2 && answerGiven == false) {
            ...
            for (var j = 1; j <= selecteditems; j++) {
                $("#dropdown_" + j).find("ul li:first .optHTxt").html(selectOptTxt);
            }
            fnselectreset();
			submitEnable=false;
			$("#submit").addClass("cyudisabled");
...
function fnLoadQuestion()
{
	...
	var loadRightImage=fnLoadCYUImage(questionImageName);
		$('.leftContent.leftipad img').attr("src",loadRightImage);
	optionArr.forEach(function(item,ind) {
		for(var i=0;i<optionArr[ind].length;i++)
		{
			...
			/*  */
			$(".draggableAreaMobile").append('<div class="mobileDragAndDrop drpHotspot"><div class="selectBoxDnD shell_bg_orange" id="dropdown_'+(i+1)+'"><span class="dropArrow"></span><div class="slct_arw"></div><ul><li class="elips Solid-BG drdn_border"><span class="drpNumber"> '+(i+1)+'.</span><span class="optHTxt">'+selectOptTxt[currentQuest]+'</span><em></em> </li><ul class="sub_slct"></ul></ul></div></div><div class="clear"></div>')
		}
	...
	if(device.MobileDevice()){
	 $(".leftContent img").removeAttr("data-action");
	 }
	 if(device.iPad()){
		   if(window.innerHeight > window.innerWidth){
		   if(optionArr[currentQuest].length > 12){
 		 $(".inner_height_drophotspot").css("overflow","auto");
 		 $(".sub_slct").css("position","relative");
 		 $(".sub_slct").css("top","0px");
		 $(".inner_height_drophotspot").mCustomScrollbar({
   ...
   $(".inner_height_drophotspot").css("overflow","");
  $(".inner_height_drophotspot").mCustomScrollbar('destroy');
 }
		 }
		 else if(window.innerWidth > window.innerHeight){
		 if(optionArr[currentQuest].length > 4){
 		 $(".inner_height_drophotspot").css("overflow","auto");
 		 $(".sub_slct").css("position","relative");
 		 $(".sub_slct").css("top","0px");
		 $(".inner_height_drophotspot").mCustomScrollbar({
...
function matchCorrectAns_1() {
    for (var j = 0; j < selecteditems; j++) {
        $("#dropdown_" + (j + 1)).find("ul li:first .optHTxt").html(optionArr[currentQuest][cAns[j]]);
        $("#dropdown_" + (j + 1)).find("ul li:first .optHTxt").css("textOverflow ", "ellipsis");
    }
}
    ...
    if (counter == selecteditems) {
		...
		if(aAttempt==2){
			...
			for (var i = 0; i <= selecteditems; i++) {
				...
				$("#dropdown_" + i).find("ul li:first .optHTxt").html(selectOptTxt);
...
function CorrectAns() {
    for (var j = 0; j < selecteditems; j++) {
        $("#dropdown_" + (j + 1)).find("ul li:first .optHTxt").html($("#dropdown_" + (j + 1)).find("ul li").eq(cAns[j] + 1).html());
        $("#dropdown_" + (j + 1)).find("ul li:first .optHTxt").css("textOverflow ", "ellipsis");
    }
}
function Feedback() {
        ...
        $("#dropdown_" + j).bind("click", function(e) {
            ...
            if (visited == false) {
                ...
                $("#dropdown_" + i).find(".sel").click(function(e) {
                    ...
                    $("#dropdown_" + i).find("ul li:first .optHTxt").html(tempval);
					$(".slct_arw").hide();
					$(".dropArrow").css("z-index","99");
                    fnreset();
                    fnsubmitEnable();
...
function fnsubmitEnable() {
	console.log("::::::::::::::cAns.length::::::::"+cAns.length);
    var subCnt = 0;
    for (var j = 0; j < cAns.length; j++) {
